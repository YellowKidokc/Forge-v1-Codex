  ...
  const sendMessage = async (messageText: string, forcedMode?: AiMode): Promise<boolean> => {
    ...
    await runAiRoleChat(
      ...
      chatForModel,
      {
        ...
        onComplete: (full) => {
          const roleConfig = getRoleConfig(activeMode);
      ...
      controller.signal,
      aiUseWorkspaceContext && workspaceContext ? `Workspace context:\n${workspaceContext}` : ''
    );
    ...
    <div className="ai-panel bg-[#161616] border-l border-[#2a2a2a] w-96 h-screen flex flex-col">
      ...
      <div className="px-3 py-2 border-b border-[#222] flex items-center gap-2">
        ...
        <div className="ml-auto text-[10px] text-gray-500">
          {roleRouting === 'shared' ? 'shared engine' : 'split engines'}
        </div>
      </div>
      ...
      <div className="border-b border-[#222] p-3 space-y-2">
        <div className="flex items-center justify-between">
          <p className="text-[10px] uppercase tracking-widest text-gray-500">Runtime Feed</p>
          <span className="text-[10px] text-gray-600">{roleRouting === 'shared' ? 'shared' : 'split'}</span>
        </div>
        <div className="space-y-1 max-h-36 overflow-y-auto">
          ...
          {runtimeEvents.map((event) => (
            <div key={event.id} className="rounded border border-forge-steel px-2 py-1.5 bg-black/20">
      ...
      <div className="p-3 border-t border-[#222]">
        {!canSend && (
          <p className="text-[11px] text-red-400 mb-2">{missingKeyMessage(provider)}</p>
        )}
        <p className="text-[10px] text-gray-500 mb-2">Provider: {providerLabel(provider)}</p>
