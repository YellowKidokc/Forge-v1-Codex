      ...
      {deps.length > 0 && (
        <section className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500">Depends On ({deps.length})</span>
          </div>
          {deps.map((d: any) => (
            <Card key={d.id} className="p-3 border-slate-800/40 bg-slate-900/10 hover:border-blue-500/20 transition-all cursor-pointer" onClick={() => onNavigate?.(d.dependsOnNodeId)}>
              <span className="text-[9px] font-mono text-slate-600 block mb-1">{d.dependsOnNodeId}</span>
              {d.description && <p className="text-[10px] text-slate-400 leading-relaxed">{d.description}</p>}
            </Card>
          ))}
        </section>
      )}
      ...
      {enables.length > 0 && (
        <section className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500">Enables ({enables.length})</span>
          </div>
          {enables.map((e: any) => (
            <Card key={e.id} className="p-3 border-slate-800/40 bg-slate-900/10 hover:border-emerald-500/20 transition-all cursor-pointer group" onClick={() => onNavigate?.(e.enablesNodeId)}>
              ...
              {e.description && <p className="text-[10px] text-slate-400 leading-relaxed">{e.description}</p>}
            </Card>
          ))}
        </section>
      )}
